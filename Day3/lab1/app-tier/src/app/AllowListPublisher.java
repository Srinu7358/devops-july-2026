...
/**
 * The push plugin. The app tier decides which of its hosted paths are public,
 * generates the allow-list, and pushes it to the web tier on startup.
 */
@WebListener
    ...
    public void contextInitialized(ServletContextEvent ev) {
        ...
        try {
            ...
            ctx.log("push plugin: initial push failed (" + e.getMessage()
                    + "). Trigger /admin/publish once the web tier is up.");
        }
    }

    /** Generate the allow-list from the app tier's public-path declaration. */
    static String push(ServletContext ctx) throws Exception {
        String webTierUrl = param(ctx, "webTierPushUrl",
                "http://127.0.0.1:8080/admin/allowlist");
